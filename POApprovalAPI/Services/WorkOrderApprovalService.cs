...
public class WorkOrderApprovalService
{
    ...
    public WorkOrderApprovalService(DatabaseService database, EmailService emailService)
    {
        _database = database;
        ...
        string? requiredUserName = null)
    {
        ...
        try
        {
            ...
            if (wo == null)
                return Fail(transId, null, "Work order approval row not found");

            string poNo = Convert.ToString(wo.PoNo) ?? "";
            string approvalName = Convert.ToString(wo.ApprovalName) ?? "";
            string status = Convert.ToString(wo.Status) ?? "";
