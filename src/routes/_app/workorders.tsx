...
function PendingList() {
  ...
  function selectCardAtPoint(x: number, y: number) {
    ...
    const card = element?.closest<HTMLElement>("[data-workorder-select-id]");
    if (!card) return;
    const id = Number(card.dataset.workorderSelectId);
  ...
  async function runBulkApprove() {
    ...
    try {
      const response = await fetch(getApiUrl("/api/WorkOrder/approve-bulk"), {
        method: "POST",
      ...
      if (okCount > 0 && failCount === 0) {
        toast.success(`Approved ${okCount} work order${okCount === 1 ? "" : "s"}`);
      } else if (okCount > 0 && failCount > 0) {
        toast.warning(`Approved ${okCount}, failed ${failCount}`);
        const reasons = (result.failed as { poNo?: string; PoNo?: string; reason?: string }[])
          .slice(0, 3)
          .map((f) => `${f.poNo ?? f.PoNo ?? "WO"}: ${f.reason ?? "failed"}`)
          .join(" · ");
            ...
            if (selectMode) {
              return (
                <button
                  key={`${p.PoNo}-${transId}`}
                  type="button"
                  data-workorder-select-id={transId}
                  disabled={!canSelect}
            ...
            return (
              <Link
                ...
                onClick={() =>
                  setApprovalListNav(
                    "workorder",
                    filtered.map((row) => row.PoNo).filter(Boolean),
                  )
                }
        ...
        >
          ...
          >
            <h3 className="text-base font-semibold">
              Approve {selected.size} work order{selected.size === 1 ? "" : "s"}?
            </h3>
            <p className="mt-2 text-sm text-muted-foreground">
              Each work order is approved with the same rules as single approve. Already-processed
              rows will be reported as failed without stopping the rest.
            </p>
            <textarea
              value={bulkRemarks}
