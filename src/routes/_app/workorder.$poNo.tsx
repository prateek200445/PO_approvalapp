  ...
  useEffect(() => {
    ...
    const loadPdf = async () => {
      try {
        ...
        const pdfUrl = getApiUrl(`/api/pdf/workorder?poNo=${encodeURIComponent(poNo)}`);
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error((err as { error?: string } | null)?.error ?? "Failed to reject work order");
      }
  ...
  {woDetails?.FirmName}
      ...
      <div className="grid gap-5 lg:grid-cols-3 min-w-0 w-full">
        <div className="space-y-5 lg:col-span-2 min-w-0 w-full">
          ...
          {/* Section B: PDF */}
          <Section title="Work Order Document">
            {pdfLoading ? (
