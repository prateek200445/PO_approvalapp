...
import { ArrowLeft, Download, FileSpreadsheet, Landmark, Loader2, RefreshCw } from "lucide-react";
...
export const Route = createFileRoute("/_app/bank-sales-profile")({
  head: () => ({ meta: [{ title: "Sales Profile — Banking — PO Portal" }] }),
  component: BankSalesProfilePage,
});
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <div className="flex items-center gap-2 text-primary">
            ...
            <p className="text-xs font-semibold uppercase tracking-wide">Banking</p>
          </div>
          <h1 className="mt-1 text-2xl font-semibold tracking-tight md:text-3xl">Sales Profile</h1>
            ...
            <table className="w-full min-w-[32rem] border-collapse text-sm">
              ...
              <thead>
                <tr className="bg-[#0B3A5B] text-white">
                  <th rowSpan={2} className="border border-[#083049] px-3 py-2.5 font-semibold align-middle">
                    Sr. No.
                  </th>
                  <th rowSpan={2} className="border border-[#083049] px-3 py-2.5 text-left font-semibold align-middle">
                    Revenue Streams
                  </th>
                  <th colSpan={2} className="border border-[#083049] px-3 py-2.5 text-center font-semibold">
                    {profile?.periodLabel ?? "—"}
                  </th>
                </tr>
                <tr className="bg-[#1565A8] text-white">
                  <th className="border border-[#0B4F86] px-3 py-2.5 font-medium">Amt (INR Cr)</th>
                  <th className="border border-[#0B4F86] px-3 py-2.5 font-medium">% Share</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((row) => (
                  <tr key={row.name} className="odd:bg-background even:bg-muted/30">
                    <td className="border border-border px-3 py-2.5 text-center">{row.sr}</td>
                    <td className="border border-border px-3 py-2.5">{row.name}</td>
                    <td className="border border-border px-3 py-2.5 text-right tabular-nums">
                      {formatMn(row.amount)}
                    </td>
                    <td className="border border-border px-3 py-2.5 text-right tabular-nums">
                      {formatMn(row.share)}%
                ...
                <tr className="bg-amber-200/80 font-semibold text-slate-900">
                  ...
                  <td className="border border-border px-3 py-3">Total</td>
                  <td className="border border-border px-3 py-3 text-right tabular-nums">
